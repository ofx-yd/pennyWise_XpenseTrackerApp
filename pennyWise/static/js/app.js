  ...
  if (kind === 'debt') {
    ...
    html = `<form onsubmit="submitPlan(event,'debt',${existing?.id||'null'})">
      ...
      <div class="form-group" style="margin-bottom:14px">
        ...
        <select name="account" class="form-input">
          <option value="card"    ${existing?.account==='card'?'selected':''}>💳 Card</option>
          <option value="cash"    ${existing?.account==='cash'?'selected':''}>💵 Cash</option>
          <option value="savings" ${existing?.account==='savings'?'selected':''}>🏦 Savings</option>
        </select>
      </div>
...
function submitPlan(e, kind, editId) {
  ...
  // DEBT → credit borrowed amount as income to chosen account
  if (kind === 'debt' && !editId) {
...
function renderDebtSection() {
  ...
  el.innerHTML = debts.map(d => {
    ...
    return planCard(d,'debt',
      ...
      + (d.account ? ` · Credited to ${ACC_NAMES[d.account]}` : '')
    );
  }).join('');
...
function saveLocal() { localStorage.setItem('budgets',JSON.stringify(budgets)); localStorage.setItem('savingsGoals',JSON.stringify(savingsGoals)); localStorage.setItem('debts',JSON.stringify(debts)); localStorage.setItem('accTransfers',JSON.stringify(transfers)); }
...
// ─── INIT ───
loadCategories(); loadCategories2(); render();
